...
__global__ void recalculatePriority(int * priority, uint8_t * gray, int * route, int width, int height)
{
    ...
    if(r < height)
    {
        int c = threadIdx.x + route[r] - 2;
        if(c >= 0 && c < width)
        {
            int x = 0, y = 0;
            // for (int filterC = 0; filterC < 3; ++filterC) 
            // {
            //     uint8_t closest = s_gray[(threadIdx.y + 1) * smemWidth + threadIdx.x + filterC + 1];
            //     size_t filterIdx = 3 + filterC;
            //     x += closest * d_xSobel[filterIdx];
            //     y += closest * d_ySobel[filterIdx];
            // }
            // for (int filterC = 0; filterC < 3; ++filterC) 
            // {
            //     uint8_t closest = s_gray[threadIdx.y * smemWidth + threadIdx.x + filterC + 1];
            //     if (r > 0)
            //         closest = s_gray[threadIdx.y * smemWidth + threadIdx.x + filterC + route[r] - route[r - 1] + 1];
            //     size_t filterIdx = filterC;
            //     x += closest * d_xSobel[filterIdx];
            //     y += closest * d_ySobel[filterIdx];
            // }
            // for (int filterC = 0; filterC < 3; ++filterC) 
            // {
            //     uint8_t closest = s_gray[(threadIdx.y + 2) * smemWidth + threadIdx.x + filterC + 1];
            //     if (r < height - 1)
            //         closest = s_gray[(threadIdx.y + 2) * smemWidth + threadIdx.x + filterC + route[r] - route[r + 1] + 1];
            //     size_t filterIdx = 2 * 3 + filterC;
            //     x += closest * d_xSobel[filterIdx];
            //     y += closest * d_ySobel[filterIdx];
            // }
            for (int filterR = 0; filterR < 3; ++filterR)
